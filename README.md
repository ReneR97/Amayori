...
> Dashboard with system status, quick actions, and download progress tracking.
...
- ⚙️ **One-click Engine Setup** — Downloads and installs `N_m3u8DL-RE` directly from GitHub inside the app
...
git clone https://github.com/YOUR_USERNAME/amayori.git
cd amayori
...
On the **Dashboard**, check the **System Core Status** panel. If `N_m3u8DL-RE` shows as **Missing**:
- Click **"Install N_m3u8DL-RE"** on the dashboard, or
...
3. Open Cookie-Editor and copy the value of `_domestika_session` into the first field
4. Copy the value of `_credentials_` into the second field
...
**"N_m3u8DL-RE not found" error**
→ Go to Settings and click **Install / Update** next to the engine name.

**Login popup shows a blank page or 404**
→ The correct login URL is `https://www.domestika.org/auth/login`. If the popup fails, use the manual cookie method described in the Authentication section.
...
Special thanks to [@Cybasaint](https://www.github.com/Cybasaint) for helping with the project and giving access to his Domestika account for testing.